...
/* ================= AUTH CHECK =================
   Verified against the live Firebase Auth session + the
   user's Firestore role - NOT a value trusted from
   localStorage, which anyone could edit in devtools. */
...
function logout() {
  logoutUser().finally(() => { window.location.href = 'Login.html'; });
}
window.logout = logout;

/* Dark mode is now handled globally by js/ui.js via the
   [data-theme] attribute + CSS variables in style.css. */
...
function initCharts(orders) {
  ...
  // Real sales data for the last 7 days, computed from actual orders
  const dayLabels = [];
  const dayKeys = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date();
  ...
  orders.forEach(o => {
    ...
    if (d && salesByDay.hasOwnProperty(d.toDateString())) {
      salesByDay[d.toDateString()] += (o.totalPrice || 0);
    }
  });
  const days = dayLabels;
  const salesData = dayKeys.map(k => salesByDay[k]);
...
const USD_RATE_KEY = 'riwayat_usd_rate';
let lastKnownRevenue = 0;

function getUsdRate() {
  const saved = parseFloat(localStorage.getItem(USD_RATE_KEY));
  return (saved && saved > 0) ? saved : 280; // fallback default, admin can edit
}

function refreshUsdDisplay() {
  const rate = getUsdRate();
  const usdEl = document.getElementById('stat-revenue-usd');
  if (usdEl) usdEl.textContent = `≈ $${Math.round(lastKnownRevenue / rate).toLocaleString()}`;
}

const usdRateInput = document.getElementById('usd-rate-input');
if (usdRateInput) {
  usdRateInput.value = getUsdRate();
...
/* ================= USERS =================
   Real accounts now live in Firestore's `users` collection
   (one doc per Firebase Auth uid) instead of localStorage. */
...
async function loadUsers() {
  ...
  try {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px; color: var(--text-light);">Failed to load users</td></tr>';
    return;
  }
  ...
  if (filteredUsers.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px; color: var(--text-light);">No users found</td></tr>';
    return;
  }

  tbody.innerHTML = filteredUsers.map((user, index) => {
    const date = user.createdAt?.toDate ? user.createdAt.toDate().toLocaleDateString() : 'N/A';
...
window.deleteUser = async function(userId) {
  if (!confirm('This removes their profile and app access. Their login itself must still be deleted from the Firebase Authentication console. Continue?')) return;
  try {
...
// Resize/compress large images before they're base64-encoded into Firestore.
// This is what was causing the noticeable delay when adding a product with a
// big photo straight from a phone camera (multi-MB base64 strings).
function toCompressedBase64(file, maxDimension = 900, quality = 0.75) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const reader = new FileReader();
    reader.onerror = reject;
...
form?.addEventListener("submit", async (e) => {
  ...
  let image = "";
  if (file) image = await toCompressedBase64(file);
...
const FIRESTORE_FREE_QUOTA_BYTES = 1 * 1024 * 1024 * 1024; // 1 GiB, Firestore Spark plan

function updateStorageUsedWidget(products) {
  const pctEl = document.getElementById('storage-used-pct');
  const barEl = document.getElementById('storage-used-bar');
  if (!pctEl || !barEl) return;

  // Base64 data URLs are ~4/3 the size of the raw bytes they encode.
  const totalBytes = products.reduce((sum, p) => {
    const len = p.image ? p.image.length : 0;
    return sum + Math.round(len * 0.75);
  }, 0);

  const pct = Math.min(100, Math.round((totalBytes / FIRESTORE_FREE_QUOTA_BYTES) * 100));
...
async function loadProducts() {
  ...
  try {
    ...
    updateStorageUsedWidget(snap.docs.map(d => d.data()));
...
window.exportOrders = async () => {
  let orders = currentOrdersForExport;

  // If the Orders tab hasn't been loaded yet this session, fetch fresh.
  if (!orders.length) {
    try {
  ...
  const headers = ['Order ID', 'Customer Name', 'Phone', 'Status', 'Items', 'Total (Rs)', 'Date'];
  const rows = orders.map(o => {
    ...
    return [o.id, o.name, o.phone, o.status || 'pending', itemsSummary, o.totalPrice || 0, date];
  });
